...
version: "2.1"
updated: "2026-03-23"
paired-with: "report-renderer v1.1"

---

## 文件定位

本文件是 `report-renderer` 的 Slide 輸出規格文件。v2.1 延續 v2.0 的識別色帶 / 主從版型 / 深色 Steal 頁系統，並補強**信心層級露出規則**、**最小內容密度**與**跨文件優先序**。

**優先序**：頁面映射以 `render-mapping.md` 為準；Slide 版型與技術限制以本文件為準；若兩者衝突，先保留資料誠實性，再調整版型。
...
| v2.1 | 2026-03-23 | 補強信心層級視覺規則、內容密度守門、跨文件優先序與通用化範例；移除對上游案例庫的耦合敘述。 |
...
## 3.4 信心層級露出規則（v2.1 新增）

Slide 版面追求簡潔，但**不能以簡潔為理由隱藏不確定性**。

| 狀態 | 頁面內呈現 | 補充資訊位置 |
|------|-----------|-------------|
| `confirmed` | 正文直接呈現 | 可附 `source_ref` |
| `estimated` | 正文旁必有「初步判讀」badge | 同卡片底部或右欄附 `proxy` / `assumption` |
| `unknown` | 不進正文 | 統一進 Appendix 的「待補資料」 |

**硬規則：**
- 同一頁若同時有 `confirmed` 與 `estimated`，兩者必須有明確視覺差異，至少包含 badge、邊框或底色之一。
- `estimated` 的 badge 不能只在頁尾總結一次，必須貼近對應段落或卡片。
- `unknown` 不允許以「TBD」「待確認」佔位文字留在正文區。
...
**最低內容密度守門（v2.1 新增）：**
- Content 頁若主體內容只有 1 張 card，優先合頁，不要保留大面積空白。
- Section Divider 以外的頁面，主要內容區至少應占可視高度 60%。
- 若某頁只有單一句主張，應改用 Statement page，而不是把內容硬塞進 Analysis page。
...
說明文（`sec-desc`）不只是「說明案例做了什麼」，而是要說出「這個案例最反直覺的地方是什麼」。不同案例應用不同切入角度：例如誤判風險、系統互依、組織阻力、體驗斷點或邊界條件。
...
- 左欄寬、右欄窄，視覺重心在左
- 若右欄是 `estimated`，卡片左上需加「初步判讀」badge，卡片底部需保留 1 行 `proxy / assumption`
...
- 若 Insight 來自 `estimated`，badge 放在 Insight box 右下角，不可移到頁尾
...
- 若某層 `root_evidence.*.status = unknown`，在對應欄位底部加警示條，不可用空白代替
...
- `confidence = estimated` 的 scenario，需在對應行尾加 badge 或警示符號；`boundary_status = unknown` 時右欄需顯示「邊界條件待驗證」
...
- 「初步判讀」badge：使用案例識別色或其深色版，白字，四角微圓。
- `unknown` 警示條：用低飽和琥珀或灰橘，不與主要識別色混淆。
- 同一頁的 badge 種類不超過 2 種，避免視覺噪音。
...
- [ ] 所有 `estimated` 內容皆貼近對應段落顯示「初步判讀」badge
- [ ] 所有 `unknown` 內容皆移出正文，不以 TBD / 待確認佔位
